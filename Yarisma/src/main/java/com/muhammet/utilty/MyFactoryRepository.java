...
public class MyFactoryRepository<T, ID> implements ICrud<T, ID>{
    ...
    public <S extends T> S update(S entity) {
     ...
     public Optional<T> findByEmail(String mail) {
        ...
        Root<T> root = (Root<T>)criteria.from(t.getClass());
        criteria.select(root);
        criteria.where(builder.equal(root.get("email"), mail));
        T result =  entityManager.createQuery(criteria).getSingleResult();
        return Optional.of(result);
    }
     ...
     public boolean existByEmail(String mail) {
       ...
       public List<T> findListByYarismaId(ID id) {
        CriteriaQuery<T> criteria = (CriteriaQuery<T>)builder.createQuery(t.getClass());
        Root<T> root = (Root<T>)criteria.from(t.getClass());
        criteria.select(root);
        criteria.where(builder.equal(root.get("yarisma").get("id"), id));
        List<T> result =  entityManager.createQuery(criteria).getResultList();
        return result;
    }
     ...
     public List<T> findByAd(String ad) {
        ...
        Root<T> root = (Root<T>)criteria.from(t.getClass());
        criteria.select(root);
        criteria.where(builder.equal(root.get("ad"), ad));
        List<T> result =  entityManager.createQuery(criteria).getResultList();
        return result;
    }
